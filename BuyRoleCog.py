...
class BuyRoleCog(commands.Cog):
    ...
    async def createrole(self, ctx, color, *, name):
        ...
        role = await ctx.guild.create_role(name=name, color=color, permissions=discord.Permissions(send_message=True))
    ...
    @commands.command()
    @commands.has_permissions(manage_roles=True)
    async def roleprice(self, ctx, price, *, role: discord.Role):
        with open("/Users/sethraphael/PycharmProject/Hurb/Bots/roleprice.json", "r") as f:
            roles = json.load(f)
        ...
        with open("/Users/sethraphael/PycharmProject/Hurb/Bots/roleprice.json", "w") as f:
    ...
    async def buyrole(self, ctx, *, role: discord.Role):
        ...
        if role in ctx.author.roles:
            ...
            if str(role) not in roles[str(ctx.guild)].keys():
                embed = discord.Embed(description=f"You cannot buy this role {ctx.author.mention}!", color=discord.Color.red())
            else:
                if int(money[str(ctx.author)]) <= roles[str(ctx.guild)][str(role)]:
                    embed = discord.Embed(description=f"You do not have enough money to buy this role {ctx.author.mention}!",
                                          color=discord.Color.red())
                else:
                    money[str(ctx.author)] = int(money[str(ctx.author)])
                    money[str(ctx.author)] -= int(roles[str(ctx.guild)][str(role)])
                    embed = discord.Embed(description=f"You have purchased the {role} role for ${roles[str(ctx.guild)][str(role)]}.")
